...
*        if (dbg) print*, "tphysf3=",tphys,tphysf, tsave
...
*                print*,'sep',sep,rol(1),rol(2),rad(1),rad(2)
...
*         if (k<3)print*,'wind,acc=',dmr(k)*dt,dmt(k)*dt
...
*
     ...
     &                k3*massc(k)*radc(k)*radc(k))*dspint(k)
               djorb = djorb + djt
*                print*, 'dj',djorb,radc(k),dspint(k),dt
...
*                    if (abs(dms(k))>mass(k)) stop
            ...
            mass(k) = mass(k) - dms(k)
*         print*,'acc,wind',dmt(k)*1.0d+06*dtm,dmr(k)*1.0d+06*dtm,k
...
*            print*, 'sep at 711', sep,mass(1),mass(2),jorb
...
*
     ...
     &    (mass2i.lt.0.1d0.or..not.sgl)).or.snova)then
         ...
         bpp(jp,9) = rad(2)/rol(2)
         bpp(jp,10) = 1.0
         if (dbg) print*, 'INITIAL'
         if(snova)then
            bpp(jp,10) = 2.0
            if (dbg) print*, 'KW CHANGE'
            dtm = 0.d0
            goto 4
         endif
      endif
...
*
      if(kstar(j1).eq.0.and.q(j1).gt.0.695d0)then
        if (dbg) print*, 'dynamical mass transfer'
*
...
* if primary is normal ms and secondary is nuc burn star
...
*
*        if (dbg) print*, "tphysf5=",tphys,tphysf, tsave
      if(tphys.ge.tphysf) goto 140
*
      if(change)then
         change = .false.
...
*
*         print*,rad(j2),rol(j2), rad(j2).gt.rol(j2)
...
*
               ...
               bpp(jp,9) = ngtv2
               bpp(jp,10) = 11.0
                if (dbg) print*, 'DISRUPT'
            else
               bpp(jp,10) = 9.0
                if (dbg) print*, 'NO REMNT'
            endif
         endif
